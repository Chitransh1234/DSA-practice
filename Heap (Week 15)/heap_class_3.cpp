...
//khud ka comparator function in the below some lines of code
//by taking reference from the priority queue section cpp reference
class compare
{
public:
...
void mergeKSortedArrays(int arr[][4], int n, int k, vector<int> &ans)
{
    ...
    while(!pq.empty()){
        ...
        //next element for the same row,jisme se pop kiya hai just abhi use insert bhi toh karna hai
        if(topCol+1<n){
